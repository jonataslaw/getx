...
- GetX یک راه حل فوق العاده سبک و قدرتمند برای Flutter است. این ترکیبی از مدیریت حالت (state) با کارایی بالا، تزریق وابستگی هوشمند و مدیریت مسیر (route) با سرعت بالا است

- GetX دارای 3 اصل اساسی است. این بدان معناست که این موارد برای همه منابع موجود در کتابخانه اولویت دارند: **پرفرمنس (performance)، عملکرد و سازماندهی.**

  - **پرفرمنس (performance):** GetX بر عملکرد و مصرف حداقل منابع متمرکز است. GetX از Streams یا ChangeNotifier استفاده نمی کند.

  - **عملکرد:** GetX از سینتکس (syntax) آسان و دلپذیر استفاده می کند. مهم نیست که چه کاری می خواهید انجام دهید، همیشه یک راه آسان تر با GetX وجود دارد. این باعث صرفه جویی در ساعت ها در توسعه می شود و حداکثر عملکردی را که برنامه شما می تواند داشته باشد را ارائه می دهد.

    به طور کلی، توسعه دهنده باید نگران حذف کنترلرها از حافظه باشد. در GetX این ضروری نیست زیرا منابع زمانی که به طور پیش فرض استفاده نمی شوند از حافظه حذف می شوند. اگر می خواهید آن را در حافظه نگه دارید، باید ویژگی "permanent: true" را در وابستگی خود اعلام کنید. به این ترتیب، علاوه بر صرفه جویی در زمان، کمتر در معرض خطر وابستگی غیر ضروری به حافظه قرار خواهید داشت. بارگذاری وابستگی نیز به طور پیش فرض به صورت lazy است.

  - **سازماندهی:** GetX امکان جداسازی کامل View، منطق ارائه (presentation logic)، منطق تجاری (business logic)، تزریق وابستگی و ناوبری (navigate) را فراهم می کند. برای جابه‌جایی بین صفحه‌ها نیازی به context ندارید، بنابراین برای این کار به درخت ویجت‌ها وابسته نیستید. شما برای دسترسی به کنترلرها یا بلاک‌ها از طریق inheritedWidget نیازی به context ندارید؛ بنابراین منطق ارائه (presentation logic)، منطق تجاری (business logic)، را کاملاً از لایه تجسم (visualization) جدا می‌کنید. شما نیازی به تزریق کلاس های Controller/Models/Blocks به درخت ویجت خود از طریق `MultiProvider` ندارید. برای این کار، GetX از ویژگی تزریق وابستگی خود استفاده می کند و DI را به طور کامل از view خود جدا می کند.

    با GetX می‌دانید که هر یک از قابلیت‌های برنامه‌تان را کجا پیدا کنید، به‌طور پیش‌فرض کد تمیزی دارید. علاوه بر آسان کردن تعمیر و نگهداری، به اشتراک گذاری ماژول ها چیزی است که تا آن زمان در Flutter غیرقابل تصور بود، چیزی که الان کاملاً ممکن شده است.
    BLoC نقطه شروعی برای سازماندهی کد در Flutter بود، که business logic را از visualization جدا می کند. GetX یک تکامل طبیعی از این است، نه تنها business logic ، بلکه presentation logic را نیز جدا می کند. حتی تزریق وابستگی‌ها و مسیرها (routes) نیز جدا شده‌اند و لایه داده از همه آنها خارج است. شما می دانید که همه چیز کجاست، و همه اینها به آسان‌تر از ساختن یک hello world است.
    GetX ساده‌ترین، کاربردی‌ترین و مقیاس‌پذیرترین راه برای ساخت برنامه‌های کاربردی با پرفرمنس (performance) بالا با Flutter SDK است. این یک اکوسیستم بزرگ در اطراف خود دارد که کاملاً با هم کار می کند که برای مبتدیان آسان و برای متخصصان دقیق است. ایمن، پایدار، به‌روز است و طیف وسیعی از APIهای داخلی را ارائه می‌کند که در Flutter SDK پیش‌فرض وجود ندارند.

- GetX سنگین نیست. دارای بسیاری از ویژگی ها است که به شما امکان می دهد بدون نگرانی از چیزی شروع به برنامه نویسی کنید، اما هر یک از این ویژگی ها در ظرف های جداگانه ای قرار دارند و تنها پس از استفاده اجرا می شوند. اگر فقط از State Management استفاده می کنید، فقط State Management کامپایل می شود. اگر فقط از routesاستفاده کنید، چیزی از State Management کامپایل نخواهد شد.

- GetX دارای یک اکوسیستم بزرگ، یک جامعه بزرگ، تعداد زیادی از همکاران است و تا زمانی که Flutter وجود دارد حفظ خواهد شد. GetX نیز قادر است با همان کد روی اندروید، iOS، وب، مک، لینوکس، ویندوز و سرور شما اجرا شود.
  **این امکان وجود دارد که به طور کامل از کدهای ساخته شده در فرانت اند در بک‌اند خود نیز دوباره استفاده کنید [Get Server](https://github.com/jonataslaw/get_server)**.

**علاوه بر این، کل فرآیند توسعه را می توان به طور کاملا خودکار، هم در سرور و هم در فرانت با [Get CLI](https://github.com/jonataslaw/get_cli) اجرا کرد**.

**علاوه بر این، برای افزایش بیشتر بهره وری شما، ما 
[extension برای VSCode](https://marketplace.visualstudio.com/items?itemName=get-snippets.get-snippets) و  [extension برای Android Studio/Intellij](https://plugins.jetbrains.com/plugin/14975-getx-snippets) را داریم**
